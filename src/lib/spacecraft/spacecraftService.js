...
export async function getActiveSpacecraft() {
  const cached = getWithTTL(SPACECRAFT_CACHE_KEY)
  if (cached) {
  ...
  try {
    ...
    setWithTTL(SPACECRAFT_CACHE_KEY, spacecraft, SPACECRAFT_CACHE_TTL)
    
    return { spacecraft, fromCache: false }
  } catch (error) {
    console.error('Error fetching spacecraft:', error)
    const stale = getWithTTL(SPACECRAFT_CACHE_KEY, true)
    if (stale) {
...
export async function getSpacecraftById(id) {
  ...
  try {
    ...
    setWithTTL(cacheKey, spacecraft, SPACECRAFT_DETAIL_CACHE_TTL)
