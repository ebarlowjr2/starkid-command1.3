...
export async function getActiveRockets() {
  const cached = await getWithTTL(ROCKETS_CACHE_KEY)
  if (cached) {
  ...
  try {
    ...
    await setWithTTL(ROCKETS_CACHE_KEY, rockets, ROCKETS_CACHE_TTL)
    
    return { rockets, fromCache: false }
  } catch (error) {
    console.error('Error fetching rockets:', error)
    const stale = await getWithTTL(ROCKETS_CACHE_KEY, true)
    if (stale) {
...
export async function getRocketById(id) {
  ...
  try {
    ...
    await setWithTTL(cacheKey, rocket, ROCKET_DETAIL_CACHE_TTL)
