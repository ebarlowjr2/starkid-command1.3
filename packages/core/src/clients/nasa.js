...
export async function getAPOD() {
  const r = await fetch(`${BASE}/planetary/apod?api_key=${getKey()}`)
...
export async function getNEOsToday() {
  ...
  const r = await fetch(`${BASE}/neo/rest/v1/feed?start_date=${today}&end_date=${today}&api_key=${getKey()}`)
...
export async function getDonkiAlerts() {
  const r = await fetch(`${BASE}/DONKI/notifications?type=all&api_key=${getKey()}`)
...
export async function getEPICLatest() {
  const r = await fetch(`${BASE}/EPIC/api/natural?api_key=${getKey()}`)
...
export async function getRecentSolarActivity(days = 3) {
  ...
  const [flrResp, cmeResp] = await Promise.all([
    fetch(`${BASE}/DONKI/FLR?startDate=${fmt(start)}&endDate=${fmt(end)}&api_key=${getKey()}`),
    fetch(`${BASE}/DONKI/CME?startDate=${fmt(start)}&endDate=${fmt(end)}&api_key=${getKey()}`)
  ])
